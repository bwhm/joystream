...
type ContentUpdateVideoCategoryAsArgs = {
  videoCategoryId: any
  asWho: string
}


export default class UpdateVideoCategoryCommandAsSudo extends ContentDirectoryCommandBase {
  ...
  async run() {
    const { context, input } = this.parse(UpdateVideoCategoryCommandAsSudo).flags
    const args: ContentUpdateVideoCategoryAsArgs = this.parse(UpdateVideoCategoryCommandAsSudo).args as ContentUpdateVideoCategoryAsArgs

    const videoCategoryId = args.videoCategoryId
    ...
    await this.sendAndFollowNamedSudoAsTx(sudoKey, currentAccount, 'content', 'updateVideoCategory', [
      actor,
      videoCategoryId,
