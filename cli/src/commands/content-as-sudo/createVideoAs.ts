...
export default class CreateVideoCommandAsSudo extends UploadCommandBase {
  ...
  async run() {
    const args: SudoAs = this.parse(CreateVideoCommandAsSudo).args as SudoAs
    const { input, channelId } = this.parse(CreateVideoCommandAsSudo).flags

    // Get context
    const sudoKey = await this.getRequiredSelectedAccount()
    const account = args.asWho
    const channel = await this.getApi().channelById(channelId)
    const actor = await this.getChannelOwnerActorAsSudo(account, channel)
    ...
    await this.sendAndFollowNamedSudoAsTx(sudoKey, account, 'content', 'createVideo', [actor, channelId, videoCreationParameters])
